...
def topwear(request, data=None):
    ...
    if data == None:
        topwears = Product.objects.filter(category='TW')
    elif data == 'Levis' or data == 'Raymonda':
        topwears = Product.objects.filter(category='TW').filter(brand=data)
    elif data == 'below':
        topwears = Product.objects.filter(category='TW').filter(discounted_price__lt=50000)
    elif data =='above':
        topwears = Product.objects.filter(category='TW').filter(discounted_price__gt=50000)
    if request.user.is_authenticated:
        totalitem = len(Cart.objects.filter(user=request.user))
...
def bottomwear(request, data=None):
    ...
    if data == None:
        ...
        bottomwears = Product.objects.filter(category='BW').filter(discounted_price__lt=50000)
    elif data =='above':
        bottomwears = Product.objects.filter(category='BW').filter(discounted_price__gt=50000)
    if request.user.is_authenticated:
        totalitem = len(Cart.objects.filter(user=request.user))
